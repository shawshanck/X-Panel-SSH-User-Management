...
class Users_Model extends Model
{
    ...
    public function submit_ative($data_sybmit)
    {
        ...
        shell_exec("bash /var/www/html/app/Libs/sh/adduser " . $data_sybmit['username'] . " " . $user["password"]);
    ...
    public function submit_deative($data_sybmit)
    {
        ...
        shell_exec("bash /var/www/html/app/Libs/sh/userdelete " . $data_sybmit['username']);
        header("Location: users");
    }
    public function submit_index($data_sybmit)
    {
        ...
        if ($queryCount < 1) {
            ...
            if ($stmt) {
                ...
                shell_exec("bash /var/www/html/app/Libs/sh/adduser " . strtolower($data_sybmit['username']) . " " . $password);
                header("Location: users");
                return true;
            } else {
                return false;
    ...
    public function renewal_update($data_sybmit)
    {
        ...
        foreach ($user as $val) {
            ...
            shell_exec("sudo killall -u " . $username);
            shell_exec("bash /var/www/html/app/Libs/sh/adduser " . $username . " " . $val->password);
